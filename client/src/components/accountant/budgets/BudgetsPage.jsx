...
export const BudgetsPage = ({ onNavigateTab }) => {
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-5">
        <div>
          <h1 className="font-serif-luxury text-3xl sm:text-4xl text-[#141A17] tracking-tight font-bold">
            Budgets & Cost Center Analytics
          </h1>
          <p className="text-xs sm:text-sm text-[#5B6963] mt-1">
            Allocate departmental expenditure targets and monitor dynamic variances against posted accounting ledgers.
          </p>
        </div>
        ...
        <div className="w-full lg:w-auto flex justify-start lg:justify-end">
          <div className="rounded-2xl border border-[#E8E1D5] bg-white/90 overflow-hidden shadow-2xs w-full sm:w-95 md:w-105 lg:w-110 shrink-0">
            <img 
              src={budgetsBanner} 
              alt="Strategic budgets sustain growth" 
        ...
        {navigationTabs.map((tab) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                setActiveTab(tab.id);
                if (tab.id !== 'budgets' && onNavigateTab) {
